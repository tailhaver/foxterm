import aiohttp
from quart import Blueprint, render_template, request, session, redirect, url_for, make_response
...
async def login():
    return {"url": f"https://github.com/login/oauth/authorize?client_id={environ.get('GITHUB_ID')}"}
...
async def callback():
    ...
    if user_data.get('id') is not None:
        ...
        elif db_session.query(User).filter(User.login == user_data['login']).count() == 0:
            db_session.execute(update(User).where(User.id == user_data['id']).values(login=user_data['login']))
        db_session.commit()
        db_session.close()
